...
const BOOKMARKS = computed(() => [
  { label: t('Domain config'), path: '/domainConfig?links=none' },
  { label: t('Servers (config)'), path: '/domainConfig/servers?links=none&fields=name,listenPort,cluster' },
  { label: t('Server runtimes'), path: '/domainRuntime/serverRuntimes?links=none&fields=name,state,healthState' },
  { label: t('Lifecycle states'), path: '/domainRuntime/serverLifeCycleRuntimes?links=none&fields=name,state' },
  { label: t('Deployments'), path: '/domainConfig/appDeployments?links=none' },
  { label: t('JDBC resources'), path: '/domainConfig/JDBCSystemResources?links=none&fields=name' },
  { label: t('Machines'), path: '/domainConfig/machines?links=none&fields=name,machineType' },
  { label: t('Security realms'), path: '/domainConfig/securityConfiguration/realms?links=none&fields=name' },
])
  ...
  try {
    ...
    if (isWrite.value && body.value.trim()) {
      try {
        ...
        throw Object.assign(new Error(t('Request body is not valid JSON')), { status: 0 })
      }
    }
...
<template>
  <div>
    <PageHeader
      ...
      :subtitle="$t('Call any endpoint of the management API directly')"
      :help="
        $t(
    ...
    <HelpPanel id="explorer" :title="$t('How to explore the management API')">
    ...
    <div class="card mb-4 p-3">
      <div class="flex flex-wrap gap-2">
        ...
        <div class="flex min-w-0 flex-1 items-center gap-0">
          <span
            ...
            :title="$t('Fixed base of every management URL — type only the part after it')"
          >
          ...
          <input
            ...
            :placeholder="$t('/domainRuntime/serverRuntimes?links=none')"
            @keyup.enter="send"
          />
        </div>
      ...
      <div v-if="isWrite" class="mt-3">
        ...
        <p class="mt-1 text-xs text-amber-600 dark:text-amber-400">
          {{
            $t('{method} changes domain state. Configuration edits normally need an edit session under /edit.', {
              method,
            })
          }}
        </p>
      ...
      <p v-if="!isWrite" class="mt-3 text-xs text-zinc-400 dark:text-zinc-500">
        {{ $t('Examples — click one to run it, then edit the path and press Enter:') }}
      </p>
    ...
    <div v-else-if="result !== null" class="card overflow-hidden">
      <div class="flex items-center justify-between border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">
        <span class="text-xs text-zinc-500 dark:text-zinc-400">
          {{ $t('{count} lines', { count: pretty.split('\n').length })
          }}<template v-if="elapsed"> · {{ $t('{count} ms', { count: elapsed }) }}</template>
        </span>
        <button
          class="btn btn-ghost px-2 py-1 text-xs"
          :title="$t('Copy the whole JSON response to the clipboard')"
          @click="copy"
        >
          {{ $t('Copy') }}
        </button>
