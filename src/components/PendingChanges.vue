  ...
  <section v-if="changes.loaded" class="mb-4">
    ...
    >
      <p class="text-sm font-medium text-red-900 dark:text-red-200">
        {{ changes.lockOwner }} is editing this domain right now
      </p>
      <p class="mt-1 text-sm text-red-900/80 dark:text-red-200/80">
        WebLogic allows one editor at a time. You can read every setting on this page, but saving will be refused
        until they activate or discard their changes.
      </p>
    </div>
    ...
    >
      <div class="flex flex-wrap items-center gap-2">
        ...
        <p class="text-sm font-medium text-amber-900 dark:text-amber-200">
          ...
          <InfoTip
            heading="Why two steps?"
            text="WebLogic writes your changes into a pending set first. The domain keeps running with its old values until you activate them, so a half-finished set of edits never reaches a live server."
            label="Why changes have to be activated"
            tone="warn"
          />
        </p>
        ...
        <div class="ml-auto flex items-center gap-2">
          <button
            ...
            title="Throw the pending changes away and release the configuration lock. The domain keeps the values it is running with."
            @click="$emit('discard')"
          >
            {{ changes.busy === 'discarding' ? 'Discarding…' : 'Discard' }}
          </button>
          <button
            class="btn btn-primary"
            :disabled="Boolean(changes.busy)"
            title="Apply every pending change to the domain now."
            @click="$emit('activate')"
          >
            {{ changes.busy === 'activating' ? 'Activating…' : 'Activate changes' }}
          </button>
        </div>
      </div>
      ...
      <p class="mt-1 text-sm text-amber-900/80 dark:text-amber-200/80">
        <template v-if="changes.pendingKnown && count">
          {{ count }} change{{ count === 1 ? '' : 's' }} {{ count === 1 ? 'is' : 'are' }} waiting.
        </template>
        <template v-else>Changes are waiting.</template>
      ...
      <template v-if="changes.pendingKnown && count">
        ...
        >
          {{ showList ? 'Hide' : 'Show' }} what is waiting
        </button>
    ...
    >
      <span>You hold the configuration lock, but nothing is waiting to be activated.</span>
      <button
        class="btn btn-ghost ml-auto"
        :disabled="Boolean(changes.busy)"
        title="Release the configuration lock so another operator can edit the domain."
        @click="$emit('discard')"
      >
        Release the lock
      </button>
