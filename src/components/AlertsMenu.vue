...
<template>
  <div class="relative">
    <button
      ...
      :aria-label="alerts.unread ? `${alerts.unread} unread alerts` : 'Alerts'"
      :aria-expanded="open"
      title="Alerts raised by watching this domain: a server leaving RUNNING, a heap or queue past its threshold, a stuck thread. Click to see them and to set the thresholds."
      @click="toggle"
    >
      <svg
        class="h-4 w-4"
    ...
    >
      <div class="flex items-center gap-2 border-b border-zinc-200 px-3 py-2 dark:border-zinc-800">
        <p class="flex items-center gap-1 text-sm font-semibold text-zinc-900 dark:text-zinc-50">
          ...
          <InfoTip
            heading="Alerts"
            text="Raised from the runtime samples the console collects in the background, on whichever page you are on. Each one fires when the condition starts holding and again only once it has cleared, so a long-running problem is one line, not hundreds."
          />
        </p>
        <button
          class="ml-auto text-xs text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-100"
          title="Show the thresholds these alerts fire at"
          @click="showRules = !showRules"
        >
          {{ showRules ? 'Hide thresholds' : 'Thresholds' }}
        </button>
        <button
          v-if="alerts.alerts.length"
          class="text-xs text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-100"
          title="Remove every alert from this list. It does not change anything on the domain."
          @click="alerts.clear()"
        >
          Clear
        </button>
      ...
      <div v-if="showRules" class="space-y-3 border-b border-zinc-200 px-3 py-3 text-sm dark:border-zinc-800">
        <label class="flex items-center justify-between gap-2">
          <span class="text-zinc-700 dark:text-zinc-200">Server leaves RUNNING</span>
          <input
            type="checkbox"
        ...
        <label class="flex items-center justify-between gap-2">
          <span class="text-zinc-700 dark:text-zinc-200">Heap above</span>
        ...
        <label class="flex items-center justify-between gap-2">
          <span class="text-zinc-700 dark:text-zinc-200">Stuck threads at or above</span>
          <input
            class="input w-20 py-1"
            type="number"
            min="1"
        ...
        <label class="flex items-center justify-between gap-2">
          <span class="text-zinc-700 dark:text-zinc-200">Queued requests at or above</span>
          <input
            class="input w-20 py-1"
            type="number"
            min="0"
        ...
        <label class="flex items-center justify-between gap-2">
          <span class="text-zinc-700 dark:text-zinc-200">
            ...
            <span class="block text-xs text-zinc-400 dark:text-zinc-500">Also notify when this tab is hidden</span>
          </span>
          <input type="checkbox" :checked="alerts.desktop" @change="alerts.setDesktop($event.target.checked)" />
        </label>
        <button
          class="text-xs text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-100"
          :title="`Back to the defaults: heap ${DEFAULT_RULES.heapPercent}%, ${DEFAULT_RULES.stuckThreads} stuck thread, ${DEFAULT_RULES.queueLength} queued requests`"
      ...
      <p v-else class="flex-1 px-3 py-8 text-center text-sm leading-relaxed text-zinc-400 dark:text-zinc-500">
        Nothing to report. Alerts appear here when something changes — the state the domain was already in when you
        opened the console is on the Dashboard, not in this list.
      </p>
    </div>
  </div>
</template>
